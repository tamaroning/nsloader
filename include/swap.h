...
template <typename T, typename F> struct swap_struct_t {
  ...
  public:
    ...
    operator i8() const { return static_cast<i8>(swap()); }
    operator u8() const { return static_cast<u8>(swap()); }
    operator i16() const { return static_cast<i16>(swap()); }
    operator u16() const { return static_cast<u16>(swap()); }
    operator i32() const { return static_cast<i32>(swap()); }
    operator u32() const { return static_cast<u32>(swap()); }
    operator i64() const { return static_cast<i64>(swap()); }
    ...
    template <typename S> bool operator==(const S &i) const {
        return swap() == i;
    }
...
template <typename T> struct swap_64_t {
    static T swap(T x) { return static_cast<T>(NSLoader::swap64(x)); }
};

template <typename T> struct swap_32_t {
    static T swap(T x) { return static_cast<T>(NSLoader::swap32(x)); }
};

template <typename T> struct swap_16_t {
    static T swap(T x) { return static_cast<T>(NSLoader::swap16(x)); }
};

template <typename T> struct swap_float_t {
    static T swap(T x) { return static_cast<T>(NSLoader::swapf(x)); }
};

template <typename T> struct swap_double_t {
    static T swap(T x) { return static_cast<T>(NSLoader::swapd(x)); }
};
...
template <typename T> struct AddEndian<T, KeepTag> { using type = T; };

// SwapTag specializations

template <> struct AddEndian<u8, SwapTag> { using type = u8; };
...
template <> struct AddEndian<i8, SwapTag> { using type = i8; };

template <> struct AddEndian<i16, SwapTag> {
    using type = swap_struct_t<i16, swap_16_t<i16>>;
};

template <> struct AddEndian<i32, SwapTag> {
    using type = swap_struct_t<i32, swap_32_t<i32>>;
};

template <> struct AddEndian<i64, SwapTag> {
...
using LETag = std::conditional_t<std::endian::native == std::endian::little,
                                 KeepTag, SwapTag>;
using BETag = std::conditional_t<std::endian::native == std::endian::big,
                                 KeepTag, SwapTag>;
